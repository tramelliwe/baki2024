...
Baki_Scores <- select(Baki_Scores, -c(score_id, score_patient_id,, score_sapsii))
colnames(Baki_Scores) <- c("StudyID", "Days", "apii")
baki_raw <- merge(baki_raw, Baki_Scores, by=c("StudyID", "Days"), all.x=T)
...
comorb_jorien <- baki_jorien %>%
  select(c(patient,cor_art_disease,diab_mell,mech_vent_d1,cortico_home,nsaid_home, reason_adm_hosp, icu_ref_from))
...
baki_samya <- baki_samya %>%
  ...
  mutate(temp_id = paste0(StudyID,"_",Days)) %>%
  select(-c(StudyID,Days))
baki_samya$MDA <- baki_samya$MDA*5
...
data <- data_orig %>%
  ...
  summarise(Age=first(Age),
            ...
            icu_ref_from = first(icu_ref_from),
            #ER: emergency room
            #OD: Floor
            #OH: Other hospital
            #OR: Operating room
            reason_adm_hosp = first(reason_adm_hosp),
            #ES: elective sugery
            #M: Medical
            #US: Urgent surgery
...
max_d17 <- data_orig %>%
  ...
  summarise(across(all_of(biomarkers), \(x) log2(max(x, na.rm=T)+1), .names = "{.col}_max_d17"),
            ...
            apii_max_d17 = max(apii, na.rm=T),
            apii_min_d17 = min(apii, na.rm=T),
...
max_d13 <- data_orig %>%
  ...
  summarise(across(all_of(biomarkers), \(x) log2(max(x, na.rm=T)+1), .names = "{.col}_max_d13"),
            ...
            apii_max_d13 = max(apii, na.rm=T),
            apii_min_d13 = min(apii, na.rm=T),
...
max_d27 <- data_orig %>%
  ...
  summarise(across(all_of(biomarkers), \(x) log2(max(x, na.rm=T)+1), .names = "{.col}_max_d27"),
            ...
            apii_min_d27 = min(apii, na.rm=T),
            apii_max_d27 = max(apii, na.rm=T),
...
demography <- demography %>%
  ...
  summarise(number_of_measures = length(Days),
            ...
            apii_max = max(apii,na.rm = T),
            apii_min = min(apii, na.rm = T),
  ...
  mutate(apii_max = ifelse(apii_max == -Inf,  NA, apii_max)) %>%
  mutate(apii_min = ifelse(apii_min == +Inf,  NA, apii_min)) 
...
apii <- calculate_summary_stats(demography$apii_max, na.rm=T)
...
demography_table <- data.frame(
  Characteristics = c("Age, years","Median (IQR)", "Min-max range", "Gender, n (%)", "Male", "Female", "Body mass index (BMI), kg/m2", "Median (IQR)", "Min-max range", "Highest SOFA during ICU", "Median (IQR)", "Min-max range", "Highest APACHE II during ICU", "Median (IQR)","Min-max range", "Sepsis stage on day 1, n (%)","No sepsis", "Sepsis", "Septic shock", "No data","AKI stage on day 1, n (%)","No AKI", "Stage 1", "Stage 2", "Stage 3", "No data"),
  
  Total_cohort = c("",age[[1]], age[[2]], "", male_n, female_n, "", bmi[[1]], bmi[[2]], "", sofa[[1]], sofa[[2]], "", apii[[1]], apii[[2]], "",sepsis[[1]], sepsis[[2]], sepsis[[3]], sepsis[[4]],"",AKI[[1]], AKI[[2]], AKI[[3]], AKI[[4]], AKI[[5]])
)
...
#survival
ggbetweenstats(data, x=survival_30d, y = Age, type = "nonparametric") #0.0132
...
chisq.test(table(select(data,c(mech_vent_d1,sepsis_d1_shock)))) 
...
ggbetweenstats(data, x=nsaid_home, y = SOFA_d1,type = "nonparametric")#Yes
...
for (biomarker in biomarkers){
  ...
  ggsave(paste0("figures/survival/graph_surv_",biomarker,".svg"), graph, width = 5, height = 5, units = "cm")
list_graphs[[c]] <-graph
c <- c+1
...
coxph(Surv(interval_ceil30, censoring_30d) ~ cytokines_il6_max_d17+cytokines_il1b_max_d17+cytokines_tnf_max_d17+cytokines_il18_max_d17+cytokines_il10_max_d17+cytokines_il1a_max_d17+cytokines_il1ra_max_d17+MDA_max_d17+SCI_max_d17+gdf15_value_max_d17+chitinase_value_max_d17+u_chitinase_value_max_d17+Age+BMI+Gender+cor_art_disease+diab_mell+apii_max_d17, data = data) %>%
  tbl_regression(exp = TRUE)
...
model_restr
model_opt <- train(survival_30d ~ Age+BMI+Gender+cor_art_disease+diab_mell+cytokines_tnf_max_d13+cytokines_il1a_max_d13+cytokines_il1ra_max_d13+gdf15_value_max_d13+MDA_max_d13+SCI_max_d13+cytokines_il6_max_d13,method="glm",data=data_prediction,family = binomial(), trControl = ctrl )
model_opt
...
ggplot(data = data_graph, mapping = aes(x = fpr, y = sens, col = group))+
 ...
 ggsave("figures/survival/pred_surv.svg",width=7.5,height=7.5,units = "cm")


#What about the prediction for patients with established sepsis/septic shock?

data_prediction <- data %>%
  select(c(cytokines_il6_max_d13,cytokines_il1b_max_d13,cytokines_tnf_max_d13,cytokines_il18_max_d13,cytokines_il10_max_d13,cytokines_il1a_max_d13,cytokines_il1ra_max_d13,gdf15_value_max_d13,MDA_max_d13,SCI_max_d13,u_chitinase_value_max_d13,chitinase_value_max_d13,Age,BMI,Gender,cor_art_disease,diab_mell,sepsis_max_d13,survival_30d))
data_prediction <- na.exclude(data_prediction)
...
model_restr <- glm(survival_30d ~ Age+BMI+Gender+cor_art_disease+diab_mell+sepsis_max_d13 , data = data_prediction,family="binomial")

model_opti <- step(model_restr, direction="forward",scope = formula(~cytokines_il6_max_d13+cytokines_il1b_max_d13+cytokines_tnf_max_d13+cytokines_il18_max_d13+cytokines_il10_max_d13+cytokines_il1a_max_d13+cytokines_il1ra_max_d13+gdf15_value_max_d13+MDA_max_d13+SCI_max_d13+u_chitinase_value_max_d13+chitinase_value_max_d13+Age+BMI+Gender+cor_art_disease+diab_mell+sepsis_max_d13))

model_complete <- train(survival_30d ~ . , data = data_prediction,method="glm", family = binomial(), trControl = ctrl)
model_complete
model_restr <- train(survival_30d ~ Age+BMI+Gender+cor_art_disease+diab_mell+sepsis_max_d13 , data = data_prediction,method="glm", family = binomial(), trControl = ctrl)
model_restr
model_opt <- train(survival_30d ~ Age+BMI+Gender+cor_art_disease+diab_mell+sepsis_max_d13+ 
                     cytokines_il1ra_max_d13 + MDA_max_d13 + cytokines_il1a_max_d13 + gdf15_value_max_d13 + cytokines_tnf_max_d13 + cytokines_il6_max_d13 + SCI_max_d13 + cytokines_il1b_max_d13
                     , method="glm",data=data_prediction,family = binomial(), trControl = ctrl )
model_opt
...
ggplot(data = data_graph, mapping = aes(x = fpr, y = sens, col = group))+
 ...
 ggsave("figures/survival/pred_surv_sepsisadj.svg",width=7.5,height=7.5,units = "cm")
...
colnames(corr_data) <- c("IL6","IL1β","TNF","IL18","IL10","IL1ɑ","IL1ra","GDF15","MDA","$Fe[c]","u-CHI3L1","p-CHI3L1","SOFA","APII","Age","BMI")  
...
for (biomarker in biomarkers){
  ...
  smooth.line.args = list(col="darkred",
                          fill="darkred",
                          method="lm"))
  ggsave(paste0("figures/sofa/",biomarker,".svg"), graph, width = 5, height = 5, units = "cm")
  list_graphs[[c]] <- graph
  c <- c+1
}
#ggsave(paste0("figures/sofa/","biomarkers","_vs_SOFA.png"), grid.arrange(grobs = list_graphs, nrow = 3, ncol = 4),width = 6000, height = 3000, units = "px") 
...
model_opt <- train(SOFA_max_d27 ~ Age + BMI + Gender + diab_mell + cor_art_disease  + 
                     SCI_d1 + chitinase_value_d1 + MDA_d1 + cytokines_il6_d1, data = data_prediction,method="lm",trControl = ctrl)
model_opt
...
for (biomarker in biomarkers){
  ...
  c <- c+1
 ggsave(paste0("figures/sepsis/graph_sepsis_",biomarker,".svg"), graph, width = 5, height = 5, units = "cm")
print(graph)
...
model_restr
model_opt <- train(sepsis_max_d27_shock ~ cytokines_tnf_d1+cytokines_il10_d1+cytokines_il1a_d1+cytokines_il1ra_d1+Age+BMI+Gender+cor_art_disease+diab_mell,method="glm",data=data_prediction,family = binomial(), trControl = ctrl )
model_opt
...
ggsave("figures/sepsis/pred_sepsis.svg",width=7.5,height=7.5,units = "cm")
...
c <- c+1
print(graph)
 ggsave(paste0("figures/AKI/",biomarker,".svg"), graph, width = 5, height = 5, units = "cm")
...
### 1.2.5 APACHE II
```{r APACHE II, echo=TRUE, fig.width=15, message=FALSE, warning=FALSE, results='hide'}
c <- 1
list_graphs <- list()
for (biomarker in biomarkers){
  graph <- ggscatterstats(data = data, x = apii_max_d17, y = !!paste0(biomarker,"_max_d17"), type = "nonparametric",
                          ylab = paste0("log2(",biomarker_names[which(biomarker == biomarkers)],"^max)"),
                          xlab = ("apii^max"),
                          ggtheme = ggthemes::theme_clean(),
                          marginal=F,
                          smooth.line.args = list(col="darkred",
                                                  fill="darkred",
                                                  method="lm"))
  ggsave(paste0("figures/apii/apii_vs_",biomarker,".svg"), graph, width = 5, height = 5, units = "cm")
  list_graphs[[c]] <- graph
  c <- c+1
}
ggsave(paste0("figures/apii/","biomarkers","_vs_APII.png"), grid.arrange(grobs = list_graphs, nrow = 3, ncol = 4),width = 6000, height = 3000, units = "px") 

#
ggplot(data, aes(x = apii_d1, fill = survival_30d))+
  geom_density(alpha=0.5, adjust=0.6)+
  labs(fill="Survived",title = "APII on day 1 vs day 30 survival")
ggplot(data, aes(x = apii_max_d17, fill = survival_30d))+
  geom_density(alpha=0.5, adjust=0.6)+
  labs(fill="Survived",title = "Max APII day 1->7 vs day 30 survival")
ggplot(data, aes(x = apii_max_d13, fill = survival_30d))+
  geom_density(alpha=0.5, adjust=0.6)+
  labs(fill="Survived",title = "Max SOFA day 1->3 vs day 30 survival")

ctrl <- trainControl(method="LOOCV",savePredictions  =T)
data_prediction <- data %>%
  select(apii_max_d27, Age , BMI , Gender , diab_mell , cor_art_disease, cytokines_il6_d1 , cytokines_il18_d1 , cytokines_il10_d1 , cytokines_il1ra_d1 , gdf15_value_d1 , chitinase_value_d1 , cytokines_il1b_d1  , cytokines_tnf_d1  ,MDA_d1,SCI_d1,u_chitinase_value_d1)


data_prediction <- na.exclude(data_prediction)

model_restr <- lm(apii_max_d27 ~ Age + BMI + Gender + diab_mell + cor_art_disease, data = data_prediction)
model_restr

model_opt <- step(model_restr,direction="forward", scope=formula(~Age + BMI + Gender + diab_mell + cor_art_disease  + cytokines_il6_d1 + cytokines_il18_d1 + cytokines_il10_d1 + cytokines_il1ra_d1 + gdf15_value_d1 + chitinase_value_d1 + cytokines_il1b_d1  + cytokines_tnf_d1  +MDA_d1+SCI_d1+u_chitinase_value_d1))
summary(model_opt)

model_complete <- train(apii_max_d27 ~ Age + BMI + Gender + diab_mell + cor_art_disease  + cytokines_il6_d1 + cytokines_il18_d1 + cytokines_il10_d1 + cytokines_il1ra_d1 + gdf15_value_d1 + chitinase_value_d1 + cytokines_il1b_d1  + cytokines_tnf_d1  +MDA_d1+SCI_d1+u_chitinase_value_d1, data = data_prediction, method="lm",trControl = ctrl)
model_complete


model_opt <- train(apii_max_d27 ~ Age + BMI + Gender + diab_mell + cor_art_disease  + SCI_d1  + cytokines_il6_d1  + cytokines_il10_d1, data = data_prediction,method="lm",trControl = ctrl)
model_opt


model_restr <- train(apii_max_d27 ~ Age + BMI + Gender + diab_mell + cor_art_disease, data = data_prediction,method="lm",trControl = ctrl)
model_restr
...
model_data <- data.frame(
  obs = c(model_opt$pred$obs, model_restr$pred$obs),
  fitted_values = c(model_opt$finalModel$fitted.values, model_restr$finalModel$fitted.values),
  model_type = factor(rep(c("Optimized (IL6, IL10, and catalytic iron)", "Restricted (patient characteristics \n and comorbidities)"), 
                          c(length(model_opt$pred$obs), length(model_restr$pred$obs)))))
...
#selecting models with less parameters to fit high dimensional data better
simple_models <- c("EII", "VII", "EEI", "VEI")

scaled <- scale(data_clustering[,biomarkers_d1])
pca <- princomp(scaled, )
summary(pca)
...
BIC_noselect <- mclustBIC(pca$scores,modelNames = simple_models)
plot(BIC_noselect)
#backward selecting principal components, requiring a minimum BIC difference for removing the feature
s <- clustvarsel(pca$scores,search="greedy",direction="backward",BIC.diff = 80, emModels2 = simple_models)
plot(s$model$BIC)
BIC_select <- s$model$BIC
(s$model$parameters$mean)
#selected PR1,2,4
BIC <- mclustBIC(pca$score[,c(1,2,4)],modelNames = simple_models)
plot(BIC)
clust <- Mclust(pca$score[,c(1,2,4)], x=BIC, G=5)
data_clustering$classification <- factor(clust$classification)


BIC_noselect <- data.frame(cluster = rownames(BIC_noselect),
                           EII=BIC_noselect[,"EII"],
                           VII = BIC_noselect[,"VII"],
                           EEI = BIC_noselect[,"EEI"],
                           VEI = BIC_noselect[,"VEI"])
BIC_noselect <- pivot_longer(BIC_noselect, cols = c(EII, VII, EEI, VEI))

BIC_select <- data.frame(cluster = rownames(BIC_select),
                         EII=BIC_select[,"EII"],
                           VII = BIC_select[,"VII"],
                           EEI = BIC_select[,"EEI"],
                           VEI = BIC_select[,"VEI"])
BIC_select <- pivot_longer(BIC_select, cols = c(EII, VII, EEI, VEI))

ggplot(data = BIC_noselect, mapping = aes(x = as.integer(cluster), y=value, col = as.factor(name)))+
  geom_line()+
  geom_point()+
  scale_x_continuous(n.breaks = 9)+
  ggthemes::theme_clean()
ggsave(paste0("figures/mclust_corr_noselect/BIC_noselect.svg"), width = 15, height = 7.5, units = "cm")  

ggplot(data = BIC_select, mapping = aes(x = as.numeric(cluster), y=value, col = as.factor(name)))+
  geom_line()+
  geom_point()+
  scale_x_continuous(n.breaks = 9)+
  ggthemes::theme_clean()
ggsave(paste0("figures/mclust_corr_noselect/BIC_select.svg"), width = 15, height = 7.5, units = "cm")  



c <- 0
for (biomarker in biomarkers_d1){
  graph <- ggbetweenstats(data_clustering, x=classification, y = !!biomarker,
                          ...
                          xlab = "Cluster",
                          p.adjust.method = "BH",
                          ggsignif.args = list(textsize=0.8),
  ...
  c <- c+1
  graph
   ggsave(paste0("figures/mclust_corr_noselect/",biomarker,".svg"), width = 4.4, height = 7.5, units = "cm")  
  
  ggsave(paste0("figures/mclust_corr_noselect/",biomarker,"_graph.png"), graph,width=5.6, height=7.39)
}
...
survfit2(Surv(interval_ceil30, censoring_30d) ~ as.factor(classification), data = data_clustering) %>%
 ...
 ggsave("figures/mclust_corr_noselect/km_classification.svg",width=10,height=7,units="cm")

#data_clustering$classification <- relevel(data_clustering$classification, ref=1)
...
coxph(Surv(interval_ceil30, censoring_30d) ~ as.factor(classification)+Age+Gender+BMI+diab_mell+cor_art_disease+apii_d1, data = data_clustering) %>%
  tbl_regression(exp = TRUE)
